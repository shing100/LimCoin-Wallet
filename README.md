...
**기본값은 노드가 권한 값이다** (`/info` 의 `recommendedFeePerByte`). 노드가
지금 mempool 을 보고 바이트당 얼마면 다음 블록에 담기는지 알려 준다.
...
노드 지갑은 `LimCoin/src/wallet.json`(0600)에 담기며 첫 실행 시 자동 생성된다.
파일 암호(scrypt + AES-256-GCM)를 걸 수 있다. **`.gitignore` 대상이고 절대
커밋하면 안 된다.** 예전 형식의 `privateKey` 파일이 남아 있으면 뜰 때 그 키를
새 지갑의 imported 로 옮겨 담는다.
